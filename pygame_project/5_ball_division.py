...
balls.append({
    ...
    "to_x": 3,      # x축 이동방향, -3 이면 왼쪽, 3 이면 오른쪽
    "to_y": -6,     # y축 이동방향
...
while running:
    ...
    weapons = list(map(lambda w: [w[0], w[1] - weapon_speed], weapons))

    # 천장에 닿은 무기 없애기
    weapons = list(filter(lambda w: w[1] > 0, weapons))
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                # 가장 작은 공이 아닐 경우 다음 단계의 공으로 나눠주기
                if ball_img_idx < 3:
                    # 현재 공 크기 정보를 가지고 옴
